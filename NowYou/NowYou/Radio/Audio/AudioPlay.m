...
-(void)retriveData
{
    FIRDatabaseReference *radioRef = [[[_rootRef child:@"Radio"] child:[NSString stringWithFormat:@"%ld", (long)self.radioId]] child:@"data"];

    [radioRef observeEventType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        if ([snapshot exists]) {
            NSDictionary *snapValue = snapshot.value;
            NSString *bufferStr = [snapValue valueForKey:@"data"];
...
-(void)stop;
{
    ...
    [[[_rootRef child:@"Radio"] child:[NSString stringWithFormat:@"%ld", (long)self.radioId]] removeAllObservers];
